...
CREATE TABLE `{$new}collections` (
  ...
  `uid` mediumint(8) NOT NULL DEFAULT '0',
  `open` tinyint(1) NOT NULL DEFAULT '0' COMMENT '0 = closed, 1 = moderated, 2 = open',
  `status` SET('H','F','P','A') NOT NULL DEFAULT 'P' COMMENT 'Hidden, Friends, Public, Archive',
  `rating` tinyint(3) NOT NULL DEFAULT '0',
  `reviews` smallint(6) NOT NULL DEFAULT '0',
...
CREATE TABLE `{$new}stories` (
  ...
  KEY `title` (`title`(50)) USING BTREE,
...
CREATE TABLE `{$new}featured` (
  ...
  `status` ENUM('1','2','3') NULL DEFAULT NULL COMMENT 'NULL = by date, 1 = manual current, 2 = manual past, 3 = upcoming',
...
CREATE TABLE `{$new}user_favourites` (
  ...
  `type` enum('AU','CO','RC','SE','ST') NOT NULL,
